...
# 19 - кол-во оставшихся блюд (для кнопки ще 5)
# 20 - Устанавливаем стр_лист для нажатия ще 5

#Кол-во оставшихся блюд
def set_str_list(chat_id, num):
...
def get_str_list(chat_id):
    with shelve.open(shelve_name) as storage:
        try:
            answer = storage[str(chat_id)][20]
            return answer
        except KeyError:
            return None

#Кол-во оставшихся блюд
def set_next_meals(chat_id, num):
    with shelve.open(shelve_name) as storage:
        try:
            plist = storage[str(chat_id)]
            plist[19] = num
...
def get_next_meals(chat_id):
    with shelve.open(shelve_name) as storage:
        try:
            answer = storage[str(chat_id)][19]
            return answer
        except KeyError:
            return None
