...
from TelegramBot import TelegramBotOld, get_busket_keyboard, get_mody_params, get_mody, TelegramBot, get_promotion
import config
import restaurants
import txt
...
from TelegramBot import get_inline_keyboard, print_busket_item, show_busket, update_busket, choose_rest_keyboard, \
    ...
    get_help_keyboard, get_settings_keyboard, get_change_start_back_keyboard, get_faivorite_keyboard, get_in_menu_keyboard, get_name_keyboard
...
def start_message(message):
    ...
    message_keyboard = types.ReplyKeyboardMarkup(
    ...
    tBot.print_keyboard_picture(
        message.chat.id, 'https://evrasia.in.ua/images/evrasiabot/start.png', message_keyboard)
    # устанавливаем состояние пользователя
    set_user(message.chat.id)
    set_state(message.chat.id, 1)
    updateLog(message.chat.id, "Бот запущен")


@tBot.bot.message_handler(func=botRestart)
def start_message(message):
    # <editor-fold desc="Формирование клавиатуры">
    # TODO Отвязать от жестких строк. Предусмотреть мультиязычность
    message_keyboard = types.ReplyKeyboardMarkup(
...
# Функция обработки выбора ресторана ввод телефона
@tBot.bot.callback_query_handler(func=choose_rest)
def rest_chosen_number(call):
    set_rest(call.message.chat.id, call.data.split('_')[1])
    setStateForWrongMessages(call.message.chat.id, 'OK')
    if get_phone(call.message.chat.id) == -1:
        message_text = 'Будь-ласка, введіть номер телефону. \nЗараз: -'
    else:
        message_text = 'Будь-ласка, введіть номер телефону. \nЗараз: ' + \
            str(get_phone(call.message.chat.id))
    keyboard = get_change_start_back_keyboard(call.message.chat.id)

    tBot.print_keyboard_message(call.message.chat.id, message_text, keyboard)
    set_state(call.message.chat.id, 5)


@tBot.bot.message_handler(content_types=['contact'])
def gotNumber(message):
    phone_usm = message.contact.phone_number
    keyboard = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
    button_phone = types.KeyboardButton(text=phone_usm)
    keyboard.add(button_phone)
    tBot.print_keyboard_message(message.chat.id, 'Ваш номер', keyboard)
...
def chooserest(call):
    ...
    if rest_zone_id == get_rest_zone(call.message.chat.id):
        return
    # Устанавливаем идентификатор зоны
    set_rest_zone(call.message.chat.id, rest_zone_id)
    ...
    for (id, name) in cursor:
        rest_adresses.append(restaurants.restaurants[id])
    ...
    for i in range(0, len(rest_list)):
        in_buttons.append(
            types.InlineKeyboardButton(text=rest_adresses[i], callback_data='rest_{0}'.format(
                str(rest_list[i][0]))))
    # </editor-fold>
    ...
    query = 'SELECT c.id, d.name FROM categories as c LEFT JOIN descriptions_socials as d ON c.id = ' \
            ...
            '\"App\\\\Models\\\\Category\" and d.lang = \"{0}\";'.format(
                language)
    cursor = db_con.cursor()
    cursor.execute(query)
    for (id, name) in cursor:
    ...
    # </editor-fold>
    inline_keyboard = get_inline_keyboard(keyboard_tuple(
        in_buttons, 1))  # inline-клавиатура с ресторанами
    # </editor-fold>
    # TODO Реализовать мультиязычность (приглашение на выбор ресторана)
    message_text = 'Будь-ласка, оберіть адресу'  # Текст для вывода информации
    tBot.update_message_text(
        call.message.chat.id, call.message.message_id, message_text, inline_keyboard)
...
def pressedStart(message):
    return message.text == '\U0001F3E0Початок' or message.text == '\U0001F3E0'


# TODO Убрать жесткую привязку к строке. Заменить на сравнение с переменной. Предусмотреть мультиязычность
# Кнопка перехода к меню
def pressedCatalog(message):
    return message.text == '\U0001F4C1Меню' or message.text == '\U0001F4C1' or message.text == 'Скасувати'
...
def pressedBasket(message):
    return message.text == '\U0001F6CDКошик' or message.text == '\U0001F6CD'

# функция вывода ещё 5 блюд


def pressed_five_more(message):
    try:
        return str(message.text[0:2]) == 'ЩЕ'
    except:
        pass
...
def chooserest(call):
    setStateForWrongMessages(call.message.chat.id, "REST")
    ...
    for i in range(0, len(restaurant_zone_list)):
        in_buttons.append(
            types.InlineKeyboardButton(text=restaurant_zone_list[i][1], callback_data='restzone_{0}'.format(
                str(restaurant_zone_list[i][0]))))
    inline_keyboard = get_inline_keyboard(keyboard_tuple(
        in_buttons, 2))  # inline-клавиатура с зонами ресторанов
    # </editor-fold>
    ...
    message_text = 'Будь-ласка, оберіть ресторан'  # Текст сообщения
    tBot.print_keyboard_message(
        call.message.chat.id, message_text, inline_keyboard)
...
def tempPressedPay(message):
    ...
    tBot.print_keyboard_message(
        message.chat.id, 'Будь-ласка, оберіть адресу:', rest_keyboard(restaurant_list))


# Функция перехода на основной экран
@tBot.bot.message_handler(func=pressedStart)
def user_message1(message):
    # SetUserInfoDB(message)
    updateLog(message.chat.id, "Начало")
    setStateForWrongMessages(message.chat.id, 'HOME')
    ...
    if getStateNew(message.chat.id) == 1:
        clear_basket(message.chat.id)
        setStateNew(message.chat.id, 0)
    ...
    tBot.print_keyboard_message(
        message.chat.id, 'Оберіть розділ', inline_keyboard)
...
# функция проверки неожиданых сообщений в "Початок"


# Функция перехода в корзину
@tBot.bot.message_handler(func=pressedBasket)
def user_pressed_basket(message):
    updateLog(message.chat.id, "Корзина")
    setStateForWrongMessages(message.chat.id, 'BASKET')
...
def user_message(message):
    updateLog(message.chat.id, "Меню")
...
def callback_basket_press(call):
    ...
    # TODO отвязать от жесткой строки. Предусмотреть мультиязычность.
    tBot.print_keyboard_message(
        call.message.chat.id, 'Кошик', message_keyboard)
...
def callback_mody_number_press(call):
    ...
    if basket != [[-1]] and len(basket) > 0:
        if basket[get_meal_index(call.message.chat.id)][7] != int(call.data.split("_")[1]):
            basket[get_meal_index(call.message.chat.id)][7] = int(
                call.data.split("_")[1])
...
def callback_basket_cross(call):
    ...
    if get_meal_index(call.message.chat.id) != 0:
        set_meal_index(call.message.chat.id,
                       get_meal_index(call.message.chat.id) - 1)
    update_busket(tBot, call.message, call.message.message_id, basket)
...
def callback_basket_up(call):
    ...
    if basket != [[-1]] and len(basket) > 0:
        ...
        if get_promotion(basket[get_meal_index(call.message.chat.id)][0]):
            basket[get_meal_index(call.message.chat.id)][3] += 2
        else:
            basket[get_meal_index(call.message.chat.id)][3] += 1
        ...
        for i in range(len(basket[get_meal_index(call.message.chat.id)][5])):
            pack_list.append([basket[get_meal_index(call.message.chat.id)][5][i][0],
                              basket[get_meal_index(
                                  call.message.chat.id)][5][i][1],
...
def callback_basket_down(call):
    ...
    if basket != [[-1]] and len(basket) > 0 and basket[get_meal_index(call.message.chat.id)][3] > 1:
...
def callback_sticks_cross(call):
    ...
    if get_sticks_count(call.message.chat.id) != 0:
        set_sticks_count(call.message.chat.id,
                         get_sticks_count(call.message.chat.id) - 1)
...
def callback_sticks_up(call):
    ...
    if basket != [[-1]] and len(basket) > 0:
        ...
        set_sticks_basket(call.message.chat.id, basket)
        update_sticks_busket(tBot, call.message,
                             call.message.message_id, basket)
...
def callback_inline(call):
    ...
    for (image,) in cursor:
        ...
        cat_list[0].append('http://evrasia.in.ua' + image)
    if check_flag != 1:
        cat_list[0].append(
    ...
    if language == 'ua':
        query = 'SELECT p2.name, p1.quality, p2.price FROM packagings as p1 LEFT JOIN packing as p2 ON ' \
                'p1.packaging_id = p2.id WHERE p1.compact = 1 AND p1.product_id = ' + \
            str_list[1] + '; '
    cursor = db_con.cursor()
    cursor.execute(query)
    ...
    if get_promotion(basket_entry_id):
        item_count = 2
    else:
        item_count = 1
    ...
    '''СОЕВЫЙ СОУС ДОБАВЛЕНИЕ ПО УМОЛЧАНИЮ
    ...
    set_full_price(call.message.chat.id, count_full_price(
        get_basket(call.message.chat.id)))
    ...
    inline_keyboard.row(types.InlineKeyboardButton(
        text='Замовити', callback_data='meal_' + str(str_list[1]) + '_0'))
    ...
    tBot.update_message_keyboard(
        call.message.chat.id, call.message.message_id, inline_keyboard)


# Функция обращения к разделу меню.
@tBot.bot.callback_query_handler(func=checkCallbac)
def callback_inline(call):
    setStateForWrongMessages(call.message.chat.id, "MEALS")
    set_next_meals(call.message.chat.id, 0)
    next_meals = get_next_meals(call.message.chat.id)
    str_list = call.data.split('_')
    print(str_list)
    set_str_list(call.message.chat.id, str_list)
    ...
    if len(cat_list) > 0:
        ...
        # TODO отвязать от жестких строк. Предусмотреть мультиязычность
        tBot.update_message_text(
            call.message.chat.id, call.message.message_id, 'Оберіть розділ', inline_keyboard)
    # </editor-fold>
    # Если нет подкатегорий и есть только товары
        ...
        query = 'SELECT products.id, price, weight FROM products LEFT JOIN (category_product) ON (' \
                'category_product.product_id = products.id) WHERE category_product.category_id = %s and online = 1 and status = 1'
        cursor = db_con.cursor()
        cursor.execute(query, (str_list[1],))
        cat_list = []
        ...
        for i in range(0, len(cat_list)):
            ...
            for (image,) in cursor:
                check_flag = 1
                cat_list[i].append('https://evrasia.in.ua' + image)
            if check_flag != 1:
                cat_list[i].append(
        ...
        rest_meals = len(cat_list) - 5  # Оставшиеся блюда

        if len(cat_list) < 6:  # Если меньше 5 блюд доступно
            # убираем кнопку ЩЕ 5 и выводим только доступные блюда
            message_keyboard = get_in_menu_keyboard(0)
            meals = len(cat_list)  # Доступные блюда

        else:
            meals = 5
            message_keyboard = get_in_menu_keyboard(meals)

        if rest_meals > 0 and rest_meals < 5 and len(cat_list) > 4:
            message_keyboard = get_in_menu_keyboard(rest_meals)
            meals = 5
        ...
        # TODO Отвязать от жесткой строки. Предусмотреть мультиязычность.
        tBot.print_keyboard_message(
            call.message.chat.id, 'Меню', message_keyboard)
        ...
        for i in range(0, meals):
            ...
            message_str = '<b>{0}</b>\n\nВага: {1} г \n<b>Ціна: {2} грн</b>\n\n {3}'.format(cat_list[i][3],
                                                                                            ...
                                                                                            cat_list[i][4])
            types.PhotoSize(cat_list[i][5], width=400, height=400)
            tBot.sendPhoto(call.message.chat.id,
                           cat_list[i][5], message_str, inline_keyboard)
            # </editor-fold>
            # Вывод сообщения с названием и описанием товара на экран

        id22 = call.message.chat.id

        # </editor-fold>
        # Вывод сообщения с названием и описанием товара на экран


@tBot.bot.message_handler(func=pressed_five_more)
def pressed_five(message):
    set_next_meals(message.chat.id, get_next_meals(message.chat.id)+5)
    setStateForWrongMessages(message.chat.id, "MEALS")

    next_meals = get_next_meals(message.chat.id)
    str_list = get_str_list(message.chat.id)
    print(str_list)
    ...
    if len(cat_list) > 0:
        ...
        # TODO отвязать от жестких строк. Предусмотреть мультиязычность
        tBot.update_message_text(
            message.chat.id, message.message_id, 'Оберіть розділ', inline_keyboard)
    # </editor-fold>
    # Если нет подкатегорий и есть только товары
    else:
        # <editor-fold desc="Получение информации о товаре из БД">
        ...
        rest_meals = len(cat_list) - next_meals
        meals = 5
        step = meals + next_meals
        message_keyboard = get_in_menu_keyboard()
        pre_last = False

        if not pre_last and rest_meals < 5:
            step = rest_meals + next_meals
            message_keyboard = get_in_menu_keyboard(0)

        elif rest_meals - 5 < 5:
            message_keyboard = get_in_menu_keyboard(rest_meals - 5)
            step = rest_meals + next_meals - rest_meals % 5
            pre_last = True

        # Вывод блюд
        for i in range(next_meals, step):
            # <editor-fold desc="Формирование встроенной клавиатуры "Заказать">
            # TODO Отвязать от жесткой строки. Предусмотреть мультиязычность.
            ...
            message_str = '<b>{0}</b>\n\nВага: {1} г \n<b>Ціна: {2} грн</b>\n\n {3}'.format(cat_list[i][3],
                                                                                            ...
                                                                                            cat_list[i][4])
            types.PhotoSize(cat_list[i][5], width=400, height=400)
            tBot.sendPhoto(message.chat.id,
                           cat_list[i][5], message_str, inline_keyboard)
        tBot.print_keyboard_message(message.chat.id, 'Меню', message_keyboard)
...
def back_to_rest(message):
    ...
    start_message_keyboard = get_start_back_keyboard()
    tBot.print_keyboard_message(
        message.chat.id, start_message_text, start_message_keyboard)
    # TODO добавить мультиязычность
    message_text = 'Будь-ласка, оберіть ресторан'  # Текст сообщения
...
def news(message):
    setStateForWrongMessages(message.chat.id, "NEWS")
    message_keyboard = get_start_keyboard()
    # TODO предусмотреть мультиязычность
    message_text = '<a href="{0}">{1}</a>'.format(
        'https://t.me/EvrasiaUkraine', 'Офіційний канал Євразії')
    tBot.print_keyboard_message_html(
        message.chat.id, message_text, message_keyboard)
    updateLog(message.chat.id, "Новости")
...
def helpb(message):
    ...
    tBot.print_keyboard_message(
        message.chat.id, message_text, message_keyboard)
    updateLog(message.chat.id, "Помощь")
...
def help_call(message):
    ...
    tBot.print_keyboard_message(
        message.chat.id, message_text, message_keyboard)
    updateLog(message.chat.id, "Звонок оператору")
...
def help_service(message):
    setStateForWrongMessages(message.chat.id, 'HELP')
    # TODO Предусмотреть мультиязычность
    message_text = txt.get_service_text()
    message_keyboard = get_start_keyboard()
    tBot.print_keyboard_message(
        message.chat.id, message_text, message_keyboard)
    updateLog(message.chat.id, "Послуга")
...
def settings_service(message):
    #updateLog(message.chat.id, "Настройки")
    setStateForWrongMessages(message.chat.id, "SETTINGS")
...
def orders_service(message):
    ...
    setStateForWrongMessages(message.chat.id, "MyORDERS")
    # TODO предусмотреть мультиязычность
    ...
    tBot.print_keyboard_message(
        message.chat.id, first_message_text, first_message_keyboard)
    if saved_orders != [[-1]]:
        ...
        for i in range(0, len(saved_orders)):
            in_buttons.append(types.InlineKeyboardButton(text=str(get_order_date(
                message.chat.id, i)), callback_data='so_{0}'.format(str(i+1))))
        # </editor-fold>
        message_keyboard = get_inline_keyboard(keyboard_tuple(in_buttons, 2))
        tBot.print_keyboard_message(
            message.chat.id, message_text, message_keyboard)
    else:
...
def settings_phone(message):
    setStateForWrongMessages(message.chat.id, "OK")
    formattedPhone = check_phone(message.text)
    if formattedPhone is False:
        message_keyboard = types.ReplyKeyboardMarkup(
            one_time_keyboard=True, resize_keyboard=True)
        tBot.print_keyboard_message(
            message.chat.id, 'Невірно введений номер', message_keyboard)
        tBot.print_keyboard_message(
            message.chat.id, "Введіть коректний номер", message_keyboard='')
    else:
        textPhone = 'Ви ввели номер: %s' % (formattedPhone)
        set_phone(message.chat.id, formattedPhone)
        # TODO вынести формирование клавиатуры в отдельный метод
        # <editor-fold desc="Формирование основной клавиатуры">
        # TODO отвязать формирование клавиатуры от жестких строк. Предусмотреть возможность смены языка
        message_keyboard = types.ReplyKeyboardMarkup(
            one_time_keyboard=True, resize_keyboard=True)
        message_keyboard.row('Вірно', 'Повторити введеня')
        message_keyboard.row("\U0001F3E0Початок")
        tBot.print_keyboard_message(
            message.chat.id, textPhone, message_keyboard)
        set_state(message.chat.id, 1)


@tBot.bot.message_handler(func=MessageNotExcpectedAtSettings)
def messageNotExpected(message):
    tBot.print_without_keyboard(
        message.chat.id, "Будьте ласкаві, оберіть налаштування ")

# Повторное введение номера


@tBot.bot.message_handler(func=pressedDeclinedPhone)
def phone_change(message):
    setStateForWrongMessages(message.chat.id, "OK")
    message_keyboard = get_start_keyboard()
    message_text = 'Будь-ласка, введіть телефон'
...
@tBot.bot.message_handler(func=pressedAcceptPhone)
def phone_accepted(message):
    message_text = 'Номер збережено'
    message_keyboard = get_start_keyboard()
    tBot.print_keyboard_message(
        message.chat.id, message_text, message_keyboard)
...
def pfaivorite(message):
    ...
    order_id = len(saved_orders)

    # Пустая корзина
    if saved_orders == [[-1]]:
        order_id = 0
    # Перезаписываем товары в полностью загруженные заказы
    elif len(saved_orders) == 4:
        if retry == 0:
            retry = 4
        order_id = len(saved_orders) - retry
        saved_orders.pop(order_id)
        saved_orders.insert(order_id, new_saved)
        set_order_date(message.chat.id, order_id)
        retry -= 1
    ...
    # Устанавливаем состояние пользователя в 1
    set_state(message.chat.id, 1)
    updateLog(message.chat.id, "Сохранение заказа")
...
def handle_message(message):
    if get_state(message.chat.id) == 5:
        ...
        message_text = 'Замовлення оформлено \nВаше замовлення на суму {0} грн відправлено \nЗ Вами зв\'яжеться оператор для уточнення деталей замовлення\nДякуємо за те, що Ви з нами!\nЗамовлення опрацьовуються оператором з 10:00 до 00:00.!'.format(
            str(int(get_full_price(message.chat.id))))

        setStateForWrongMessages(message.chat.id, "PHONE_ENTERED")
        ...
        if(message.text == '✅Вірно'):
            # Забираем номер с настроек
            user_phone = get_phone(message.chat.id)

            # проверка корректности номера
            correct_number = check_phone(user_phone)
            wrong_number = 'Невірний номер у налаштуваннях.'
            # Обратно форматируем номер телефона к исходному виду
            if correct_number:
                user_phone = reparseNumber(user_phone)

        else:
            # Иначе с сообщения
            user_phone = reparseNumber(message.text)
            correct_number = check_phone(user_phone)
            wrong_number = 'Невірно введений номер'
        ...
        if correct_number:
            ...
            db_con = connection.MySQLConnection(user=config.db_user, password=config.db_password, host=config.db_host,
                                                database=config.db_name)
            query = 'SELECT online_code FROM restaurants WHERE id = {0};'.format(
                str(get_rest(message.chat.id)))
            cursor = db_con.cursor()
            cursor.execute(query)
            for (rest_id,) in cursor:
                restaurant_id = rest_id  # идентификатор ресторана
            message_text += '\nАдреса ресторану {0}'.format(
                restaurants.restaurants[int(get_rest(message.chat.id))])
            cursor.close()
            db_con.close()
            restaurant_id = int(get_rest(message.chat.id))
            db_con = connection.MySQLConnection(user=config.db_user, password=config.db_password, host=config.db_host,
                                                database=config.db_name)
            query = 'SELECT online_code FROM restaurants where id = {}'.format(
                restaurant_id)
            cursor = db_con.cursor()
            cursor.execute(query)
            for (REST_ID,) in cursor:
                restaurant_id = REST_ID  # код продукта
            ...
            for i in range(len(basket)):
                ...
                db_con = connection.MySQLConnection(user=config.db_user, password=config.db_password, host=config.db_host,
                                                    database=config.db_name)
                query = 'SELECT global_code FROM products WHERE id = {0};'.format(
                    str(basket[i][0]))
                cursor = db_con.cursor()
                cursor.execute(query)
                ...
                order_mody = int(basket[i][6][basket[i][7]]
                                 [0]) if basket[i][6] != [-1] else 0
                order_type_pack = 0
            ...
            for i in range(len(basket)):
                ...
                order_products.append(
                    {'code': order_code, 'quantity': order_quantity, 'mody': order_mody, 'type_pack': order_type_pack})
            # информация про общую сумму заказа
            order_total = '{0:.2f}'.format(get_full_price(message.chat.id))
            order = {'id': order_id, 'type': order_type,
                     'products': order_products, 'total': order_total}
            ...
            r = requests.post(
                'http://10.20.100.200/telegram/import', data={'code': encode_data})
            updateLog(message.chat.id, "Заказ оформлен", json_data)
            print(r)
            # </editor-fold>
            saved_orders = get_saved_orders(message.chat.id)
            ...
            tBot.print_without_keyboard(message.chat.id, wrong_number)
            set_state(message.chat.id, 5)


@tBot.bot.message_handler(func=WrongMessageAtRestaurants)
def unexpectedMessage(message):
    tBot.print_without_keyboard(
        message.chat.id, "Будь ласка, оберіть ресторан")


@tBot.bot.message_handler(func=WrongMessageAtHome)
def unexpectedMessage(message):
    tBot.print_without_keyboard(
        message.chat.id, 'Будь лаcка, оберіть пункт меню')


@tBot.bot.message_handler(func=WrongMessageAtBasket)
def unexpectedMessage(message):
    tBot.print_without_keyboard(message.chat.id, 'Будь ласка, оберіть страву')


@tBot.bot.message_handler(func=WrongMessageAtMeals)
def unexpectedMessage(message):
    tBot.print_without_keyboard(message.chat.id, "Будь ласка, оберіть страви")


@tBot.bot.message_handler(func=WrongMessageAtNews)
def unexpectedMessage(message):
    tBot.print_without_keyboard(message.chat.id, 'Будь ласка, оберіть новину')


@tBot.bot.message_handler(func=WrongMessageAtHelp)
def WrongMessage(message):
    tBot.print_without_keyboard(message.chat.id, "Будь ласка, оберіть послугу")


@tBot.bot.message_handler(func=WrongMessageAtSettings)
def unexpectedMessage(message):
    tBot.print_without_keyboard(
        message.chat.id, 'Будьте ласка, оберіть налатування')


@tBot.bot.message_handler(func=WrongMessageAtMyOrders)
def messageNotExpected(message):
    tBot.print_without_keyboard(
        message.chat.id, 'Будь ласка, оберіть замовлення')


@tBot.bot.message_handler(func=WrongMessageAtPhonEntered)
def messageNotExpected(message):
    tBot.print_without_keyboard(
        message.chat.id, 'Будь ласка, оберіть пункт меню')
# </editor-fold>


tBot.bot.set_webhook(url=WEBHOOK_URL_BASE + WEBHOOK_URL_PATH,
